...
void qh_freebuffers (void) {
  ...
  if (qh half_space)
    free_ (qh half_space);
  if (qh temp_malloc)
    free_ (qh temp_malloc);
  if (qh feasible_point)      /* allocated by qh_readfeasible */
    free_ (qh feasible_point);
  if (qh feasible_string)     /* allocated by qh_initflags */
    free_ (qh feasible_string);
...
void qh_initflags(char *command) {
  ...
  while (*s) {
	...
	qh_strtod (s, &t);
      ...
      if (start < t) {
        if (!(qh feasible_string= (char*)calloc_ (t-start+1, 1))) {
...
#if qh_QHpointer
  if (!(qh_qh= (qhT *)malloc_ (sizeof(qhT)))) {
