...
int qh_readfeasible (int dim, char *remainder) {
  ...
  if (!(qh feasible_point= (coordT*)malloc_ (dim* sizeof(coordT)))) {
...
coordT *qh_readpoints(int *numpoints, int *dimension, boolT *ismalloc) {
  ...
  if (qh HALFspace) {
    qh half_space= coordp= (coordT*) malloc_ (qh normal_size + sizeof(coordT));
    if (qh CDDinput) {
  ...
  qh line= (char*)malloc_ ((qh maxline+1) * sizeof (char));
  *ismalloc= True;  /* use malloc since memory not setup */
  coords= points= qh temp_malloc= 
        (coordT*)malloc_((*numpoints)*(*dimension)*sizeof(coordT));
...
Either prepend the input with 1 point or use 'Hn,n,n'.  See manual.\n");
  ...
  if (!(qh feasible_point= (pointT*)malloc_ (dim* sizeof(coordT)))) {
