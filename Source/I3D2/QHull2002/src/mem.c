...
void qh_memfreeshort (int *curlong, int *totlong) {
  ...
  *totlong= qhmem .totlong;
  ...
  if (qhmem .LASTsize) {
    free_ (qhmem .indextable);
    free_ (qhmem .freelists);
    free_ (qhmem .sizetable);
  }
...
void qh_meminitbuffers (int tracelevel, int alignment, int numsizes, int bufsize, int bufinit) {
  ...
  qhmem.sizetable= (int *) calloc_ (numsizes, sizeof(int));
  qhmem.freelists= (void **) calloc_ (numsizes, sizeof(void *));
...
void qh_memsetup (void) {
  ...
  if (!(qhmem.indextable= (int *)malloc_((qhmem.LASTsize+1) * sizeof(int)))) {
