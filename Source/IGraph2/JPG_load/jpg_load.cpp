...
struct S_file_struct{
   ...
   S_file_struct(C_cache *ck1):
      ck(ck1),
      base_offset(ck1->tellg())
   {
   }

   long Seek(long pos){
      if(!ck->seekg(base_offset + pos))
         return -1;
      return pos;
   }
   long Read(void *buf, long sz){
                              //do not let loader read beyond size (jpg loader does try to)
      sz = Min(sz, long(ck->filesize() - ck->tellg()));
      return ck->read((char*)buf, sz);
   }
};
 ...
 */
GLOBAL(JDIMENSION) jpeg_read_scanlines (j_decompress_ptr cinfo, JSAMPARRAY scanlines,
...
void build_ycc_rgb_table (j_decompress_ptr cinfo){
  ...
  for (i = 0, x = -CENTERJSAMPLE; i <= MAXJSAMPLE; i++, x++) {
    ...
    cconvert->Cr_r_tab[i] = (int)
          RIGHT_SHIFT(FIX1(1.40200) * x + ONE_HALF, SCALEBITS);
    /* Cb=>B value is nearest int to 1.77200 * x */
    cconvert->Cb_b_tab[i] = (int)
          RIGHT_SHIFT(FIX1(1.77200) * x + ONE_HALF, SCALEBITS);
...
jinit_input_controller (j_decompress_ptr cinfo)
{
  ...
  inputctl->pub.start_input_pass = start_input_pass1;
...
LOCAL(boolean)
process_restart1 (j_decompress_ptr cinfo)
{
  phuff_entropy_ptr entropy = (phuff_entropy_ptr) cinfo->entropy;
  int ci;
...
{
  my_upsample_ptr1 upsample = (my_upsample_ptr1) cinfo->upsample;
...
{
  my_upsample_ptr1 upsample;
  int ci;
  jpeg_component_info * compptr;
  boolean need_buffer, do_fancy;
  int h_in_group, v_in_group, h_out_group, v_out_group;

  upsample = (my_upsample_ptr1)
    (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_IMAGE,
            SIZEOF(my_upsampler1));
  cinfo->upsample = (struct jpeg_upsampler *) upsample;
...
#define FIX_1_847759065a  ((INT32)  15137) /* FIX(1.847759065) */
  ...
  /* and also undo the PASS1_BITS scaling. */
   ...
   for (ctr = 0; ctr < DCTSIZE; ctr++) {
    ...
    /* The rotator is sqrt(2)*c(-6). */
      ...
      tmp0 = ((INT32) wsptr[0] + (INT32) wsptr[4]) << CONST_BITS2;
      tmp1 = ((INT32) wsptr[0] - (INT32) wsptr[4]) << CONST_BITS2;
...
#define FIX_1_847759065b  ((INT32)  15137) /* FIX(1.847759065) */
          ...
          JSAMPARRAY output_buf, JDIMENSION output_col)
{
  ...
  for (ctr = 0; ctr < 4; ctr++) {
    ...
    tmp0 = ((INT32) wsptr[0]) << (CONST_BITS3+1);
    
    tmp2 = MULTIPLY5((INT32) wsptr[2], FIX_1_847759065b)
    + MULTIPLY5((INT32) wsptr[6], - FIX_0_765366865);
          ...
          JSAMPARRAY output_buf, JDIMENSION output_col)
{
  ...
  for (ctr = 0; ctr < 2; ctr++) {
    ...
    tmp10 = ((INT32) wsptr[0]) << (CONST_BITS3+2);
    
    /* Odd part */

    tmp0 = MULTIPLY5((INT32) wsptr[7], - FIX_0_720959822) /* sqrt(2) * (c7-c5+c3-c1) */
    + MULTIPLY5((INT32) wsptr[5], FIX_0_850430095) /* sqrt(2) * (-c1+c3+c5+c7) */
    + MULTIPLY5((INT32) wsptr[3], - FIX_1_272758580) /* sqrt(2) * (-c1+c3-c5-c7) */
    + MULTIPLY5((INT32) wsptr[1], FIX_3_624509785); /* sqrt(2) * (c1+c3+c5+c7) */
          ...
          long file_offset, long byte_count)
{
  //if (fseek(info->temp_file, file_offset, SEEK_SET))
   //if(dtaSeek(info->temp_file, file_offset, DTA_SEEK_SET) == -1)
   S_file_struct *fs = (S_file_struct*)info->temp_file;
   if(fs->Seek(file_offset) == -1)
      ERREXIT(cinfo, JERR_TFILE_SEEK);
   if((size_t)JFREAD(info->temp_file, buffer_address, byte_count) != (size_t)byte_count)
...
{
  //if (fseek(info->temp_file, file_offset, SEEK_SET))
  //if (dtaSeek(info->temp_file, file_offset, DTA_SEEK_SET))
   S_file_struct *fs = (S_file_struct*)info->temp_file;
   if(fs->Seek(file_offset) == -1)
      ERREXIT(cinfo, JERR_TFILE_SEEK);
...
bool LoadJPG(C_cache &ck, dword size_x, dword size_y, void *area, byte *pal, byte bpp){
   ...
   jpeg_stdio_src(&cinfo, (int)&fs);

                              //read file header, set default decompression parameters
   jpeg_read_header(&cinfo, true);

   if(size_x!=cinfo.image_width || size_y!=cinfo.image_height || bpp!=cinfo.num_components){
   ...
   byte *bp = (byte*)area;
   while(cinfo.output_scanline < cinfo.output_height) {
      jpeg_read_scanlines(&cinfo, &bp, 1);
      bp += size_x*bpp;
   }
...
bool ReadJPGHeader(C_cache &ck, dword &size_x, dword &size_y, byte &bpp){
   ...
   size_x = cinfo.image_width;
   size_y = cinfo.image_height;
...
bool SaveJPG(C_cache *ck, dword size_x, dword size_y, const void *area, byte bpp){
   ...
   //jpeg_stdio_dest(&cinfo, h);
   //S_file_struct fs(ck);
   //jpeg_stdio_dest(&cinfo, (FILE*)&fs);
   ...
   while(cinfo.next_scanline < cinfo.image_height) {
      jpeg_write_scanlines(&cinfo, &bp, 1);
      bp += size_x*bpp;
   }
