...
static void SwapRowsAndCols(ATYPE A, int n, int i1, int i2, int nskip, bool do_fast_row_swaps){
...
static void SwapProblem(ATYPE A, dReal *x, dReal *b, dReal *w, dReal *lo,
   dReal *hi, int *p, int *state, int *findex, int n, int i1, int i2, int nskip,
   bool do_fast_row_swaps){
...
static void checkPermutations (int i, int n, int nC, int nN, int *p, int *C){
   int j,k;
   assert(nC>=0 && nN>=0 && (nC+nN)==i && i < n);
   for (k=0; k<i; k++) assert(p[k] >= 0 && p[k] < i);
   for (k=i; k<n; k++) assert(p[k] == k);
   for (j=0; j<nC; j++) {
      int C_is_bad = 1;
...
dLCP::dLCP (int _n, int _nub, dReal *_Adata, dReal *_x, dReal *_b, dReal *_w,
   ...
   assert(("slow dLCP object does not support findex array", _findex==0));
...
struct dLCP{
   ...
   float *Adata, *x, *b, *w, *lo, *hi;
   float *L, *d;				          //L*D*L' factorization of set C
   float *Dell, *ell, *tmp;
...
//----------------------------
   inline int getNub() const{ return nub; }
...
//----------------------------
   inline int numC() const{ return nC; }
   inline int numN() const{ return nN; }

//----------------------------
   inline int indexC(int i) const{ return i; }
   inline int indexN(int i) const{ return i+nC; }

//----------------------------
   inline float Aii(int i) const{ return AROW(i)[i]; }

//----------------------------
   inline float AiC_times_qC(int i, float *q) const{
      return dDot (AROW(i),q,nC);
   }

//----------------------------

   inline float AiN_times_qN(int i, float *q) const{
...
dLCP::dLCP(int _n, int _nub, dReal *_Adata, dReal *_x, dReal *_b, dReal *_w,
   dReal *_lo, dReal *_hi, dReal *_L, dReal *_d, dReal *_Dell, dReal *_ell, dReal *_tmp,
   int *_state, int *_findex, int *_p, int *_C, dReal **Arows){
...
# endif
   ...
   for(k=0; k<n; k++)
      p[k]=k;
                                    ...
                                    while (i1 > i2); 
                                    //printf ("--> %d %d\n",i1,i2);
                                    SwapProblem (A,x,b,w,lo,hi,p,state,findex,n,i1,i2,nskip, false);
                                    }
                                    }
   */
...
void dLCP::transfer_i_to_C(int i){
   ...
   if(nC > 0){
    // ell,Dell were computed by solve1(). note, ell = D \ L1solve (L,A(i,C))
      for(j=0; j<nC; j++)
         L[nC*nskip+j] = ell[j];
      dReal f = AROW(i)[i] - dDot(ell, Dell, nC);
      if(!IsAbsMrgZero2(f))
         d[nC] = 1.0f / f;
      else{
         d[nC] = 0.0f;
...
#   endif
      ...
      float f = AROW(i)[i] - dDot(ell,Dell,nC);
      if(!IsAbsMrgZero2(f))
         d[nC] = 1.0f / f;
      else{
         d[nC] = 0.0f;
   ...
   for(i=0; i<n; i++){
      ...
      if(w[i] >= 0){
         ...
         for(;;){
            ...
            if(s <= 0) {
               //dMessage (1, "LCP internal error, s <= 0 (s=%.4e)",s);
               if (i < (n-1)) {
                  dSetZero (x+i,n-i);
                  dSetZero (w+i,n-i);
               }
...
void dSolveLCP(int n, dReal *A, dReal *x, dReal *b, dReal *w, int nub, dReal *lo, dReal *hi, int *findex){
   ...
   assert (n>0 && A && x && b && w && lo && hi && nub >= 0 && nub <= n);
  ...
  // check restrictions on lo and hi
   for(k=0; k<n; k++) assert(lo[k] <= 0 && hi[k] >= 0);
# endif
      ...
      if (lo[i]==0 && w[i] >= 0) {
         ...
         for (;;) {
            ...
            for (k=0; k < lcp.numN(); k++) {
               if ((state[lcp.indexN(k)]==0 && delta_w[lcp.indexN(k)] < 0) ||
                  ...
                  if(lo[lcp.indexN(k)] == 0 && hi[lcp.indexN(k)] == 0)
                     continue;
                  if(!IsAbsMrgZero2(delta_w[lcp.indexN(k)])){
                     dReal s2 = -w[lcp.indexN(k)] / delta_w[lcp.indexN(k)];
                     if(s2 < s){
                        s = s2;
                        cmd = 4;
            ...
            if (s <= 0) {
               //dMessage (1, "LCP internal error, s <= 0 (s=%.4e)",s);
               if (i < (n-1)) {
                  memset(x+i, 0, sizeof(float)*(n-i));
                  memset(w+i, 0, sizeof(float)*(n-i));
               }
               goto done;
            }
...
inline void dClearUpperTriangle (dReal *A, int n){
  int i,j;
  int skip = dPAD(n);
  for (i=0; i<n; i++) {
